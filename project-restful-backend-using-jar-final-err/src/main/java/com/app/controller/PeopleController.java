...
public class PeopleController {
	...
	public ResponseEntity<?> getPersonDetails(@PathVariable String personName) {
		System.out.println("in get person details " + personName);
		// invoke service method
		Optional<People> optional = peopledao.findByPersonName(personName);
...
//			return new ResponseEntity<>(resp, HttpStatus.NOT_FOUND);
	...
	public ResponseEntity<?> addPerson(@RequestBody People person) {
		   System.out.println("in add person " + person);
			return new ResponseEntity<>(peopledao.save(person),HttpStatus.CREATED);
		}
	...
	public ResponseEntity<?> updatePersonDetails(@PathVariable int personId, @RequestBody People person) {
		...
		// check if project exists
				Optional<People> optional = peopledao.findById(personId);
				if (optional.isPresent()) {
	...
	public ResponseEntity<?> deletePersonDetails(@PathVariable  int personId) {
		...
		if (optional.isPresent()) {
			...
			return new ResponseEntity<>(new ResponseDTO("success","Person record deleted with ID " ,+ personId), HttpStatus.OK);
			 ...
			 throw new PersonNotFoundException("Project ID Invalid : record deletion failed");
