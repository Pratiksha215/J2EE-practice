...
//			return new ResponseEntity<>(resp, HttpStatus.NOT_FOUND);
	...
	@PutMapping("/{issueId}")
	public ResponseEntity<?> updateIssueDetails(@PathVariable int issueId, @RequestBody Projects p) {
		System.out.println("in update issues" + issueId + " " + p);
		// check if project exists
				Optional<Issues> optional = issuesdao.findById(issueId);
				if (optional.isPresent()) {
					...
					return new ResponseEntity<>(issuesdao.save(existingIssues), HttpStatus.OK);
					// save or update (insert: transient(value of ID : default
					// or non default value BUT existing on DB -- update
				} else
					throw new IssuesNotFoundException("Issue ID Invalid");
